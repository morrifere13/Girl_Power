...
// Identifiants admin
const ADMIN_EMAIL = 'admin@girlpower.org';
...
const statuts = ['En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente', 'En attente'];
...
async function login() {
    try {
        ...
        console.log('\n💡 Assurez-vous que:');
        console.log('   1. Le serveur backend est démarré (http://localhost:5000)');
        console.log('   2. Les identifiants admin sont corrects:');
...
async function seedData() {
    try {
        ...
        for (let i = 0; i < projets.length; i++) {
            const projet = {
                ...projets[i],
                centre_ids: JSON.stringify([createdCentres[i].id]),
                bailleurs: JSON.stringify([]),
                partenaires: JSON.stringify([]),
                avantages: JSON.stringify([]),
                criteres_admission: JSON.stringify([]),
                zones_intervention: JSON.stringify({ regions: [], departements: [], communes: [] }),
                pays_cible: JSON.stringify(['Côte d\'Ivoire'])
            };
        ...
        console.error('\n❌ Erreur lors du seeding:', error.response?.data || error.message);
        process.exit(1);
    }
}
