-- ==============================================================================
-- MIGRATION STAGES ET ÉVALUATIONS (sans entreprises qui existe déjà)
-- ==============================================================================
...
-- Ajouter les nouvelles colonnes (IF NOT EXISTS pour éviter les erreurs si déjà existantes)
SET @sql = 'ALTER TABLE candidates ADD COLUMN en_stage BOOLEAN DEFAULT FALSE COMMENT "Actuellement en stage"';
...
SET @sql = 'ALTER TABLE candidates ADD COLUMN nombre_stages_effectues INT DEFAULT 0 COMMENT "Total stages effectués"';
PREPARE stmt FROM @sql;
BEGIN
    DECLARE CONTINUE HANDLER FOR 1060 BEGIN END;
    EXECUTE stmt;
...
SET @sql = 'ALTER TABLE candidates ADD COLUMN dernier_stage_id INT COMMENT "Référence au dernier stage"';
PREPARE stmt FROM @sql;
BEGIN
    DECLARE CONTINUE HANDLER FOR 1060 BEGIN END;
    EXECUTE stmt;
...
SET @sql = 'ALTER TABLE candidates ADD COLUMN disponible_stage BOOLEAN DEFAULT TRUE COMMENT "Disponible pour nouveau stage"';
PREPARE stmt FROM @sql;
BEGIN
    DECLARE CONTINUE HANDLER FOR 1060 BEGIN END;
    EXECUTE stmt;
...
SET @sql = 'ALTER TABLE candidates ADD CONSTRAINT fk_candidates_derniers_stage FOREIGN KEY (dernier_stage_id) REFERENCES stages(id) ON DELETE SET NULL';
PREPARE stmt FROM @sql;
BEGIN
    DECLARE CONTINUE HANDLER FOR 1826, 1061 BEGIN END;
    EXECUTE stmt;
END;
DEALLOCATE PREPARE stmt;
